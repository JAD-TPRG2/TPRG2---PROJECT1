...
# vending_machine_JAD.py -- GUI for vending machine.
# TPRG2131 Section 02
# October 28, 2024
...
# Code adapted from code provided by: Prof. Phil Jarvis
...
class VendingMachine(object):
    ...
    # List of products: each tuple is ("NAME", "PRICE", "STOCK")
    PRODUCTS = {"suprise": ("SURPRISE", 5, 3),
                "candy": ("CANDY", 75, 5),
                "chips": ("CHIPS", 150, 5),
                "drinks": ("DRINKS", 200, 5),
                "sandwich": ("SANDWICH", 250, 5),
                }
    ...
    def sold_out(self, count):
        """Mark a product as sold out."""
        if count in self.PRODUCTS:
            inventory = list(self.PRODUCTS[count])
            inventory[2] -= 1
            if inventory[2] == 0:
                inventory[1] = 'SOLD OUT'
            self.PRODUCTS[count] = tuple(inventory)
        self.event = 'SOLD OUT'
        self.update()
...
if __name__ == "__main__":
    ...
    # Print into GUI window not SHELL
    print = sg.Print
    # Store for total coin inserted
    TOTAL = 0
    coin_col = []
    ...
    for item, item_price in VendingMachine.PRODUCTS.items():
        log(item)
        PRICES = item_price[1]
        price = sg.Text(f"${PRICES / 100:.2f}", font=("Helvetica", 18))
        button = sg.Button(item, font=("Helvetica", 18))
        row = [button, price]
        select_col.append(row)

    layout = [ [sg.Column(coin_col, vertical_alignment="TOP"),
                    sg.Column(total_col, vertical_alignment="MIDDLE"),
                     sg.VSeparator(),
   ...
   # Checks if being used on Pi
    ...
    while True:
        ...
        if event in vending.COINS:
            TOTAL += vending.COINS[event][1]
            window['total'].update(f'Total: ${TOTAL / 100:.2f}')
